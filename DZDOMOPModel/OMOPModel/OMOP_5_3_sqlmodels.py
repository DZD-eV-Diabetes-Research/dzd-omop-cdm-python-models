...
class CdmSource(SQLModel, table=True):
    ...
    source_release_date: Optional[date] = Field(default=None, sa_column=Column('source_release_date', Date), description='USER GUIDANCE: The release date of the source data.')
    cdm_release_date: Optional[date] = Field(default=None, sa_column=Column('cdm_release_date', Date), description='USER GUIDANCE: The release data of the CDM instance.')
...
class Concept(SQLModel, table=True):
    ...
    attribute_definition: List['AttributeDefinition'] = Relationship(back_populates='attribute_type_concept')
    ...
    procedure_occurrence1: List['ProcedureOccurrence'] = Relationship(back_populates='procedure_type_concept')
    ...
    visit_occurrence: List['VisitOccurrence'] = Relationship(back_populates='admitting_source_concept')
    visit_occurrence_: List['VisitOccurrence'] = Relationship(back_populates='discharge_to_concept')
    ...
    visit_detail: List['VisitDetail'] = Relationship(back_populates='admitting_source_concept')
    visit_detail_: List['VisitDetail'] = Relationship(back_populates='discharge_to_concept')
    ...
    measurement: List['Measurement'] = Relationship(back_populates='measurement_concept')
    measurement_: List['Measurement'] = Relationship(back_populates='measurement_source_concept')
    measurement1: List['Measurement'] = Relationship(back_populates='measurement_type_concept')
    measurement2: List['Measurement'] = Relationship(back_populates='operator_concept')
    measurement3: List['Measurement'] = Relationship(back_populates='unit_concept')
    measurement4: List['Measurement'] = Relationship(back_populates='value_as_concept')
    ...
    note2: List['Note'] = Relationship(back_populates='note_type_concept')
    observation: List['Observation'] = Relationship(back_populates='observation_concept')
    observation_: List['Observation'] = Relationship(back_populates='observation_source_concept')
    observation1: List['Observation'] = Relationship(back_populates='observation_type_concept')
    observation2: List['Observation'] = Relationship(back_populates='qualifier_concept')
    observation3: List['Observation'] = Relationship(back_populates='unit_concept')
    observation4: List['Observation'] = Relationship(back_populates='value_as_concept')
...
class Vocabulary(SQLModel, table=True):
    ...
    vocabulary_reference: str = Field(sa_column=Column('vocabulary_reference', String(255)), description='USER GUIDANCE: External reference to documentation or\navailable download of the about the\nvocabulary.')
...
class AttributeDefinition(SQLModel, table=True):
    ...
    attribute_definition_id: int = Field(sa_column=Column('attribute_definition_id', Integer, primary_key=True))
    attribute_name: str = Field(sa_column=Column('attribute_name', String(255)))
    attribute_type_concept_id: int = Field(sa_column=Column('attribute_type_concept_id', Integer, primary_key=True))
    ...
    attribute_type_concept: Optional['Concept'] = Relationship(back_populates='attribute_definition')
...
class CareSite(SQLModel, table=True):
    ...
    care_site_id: int = Field(sa_column=Column('care_site_id', Integer, primary_key=True), description=' | ETLCONVENTIONS: Assign an id to each unique combination of location_id and place_of_service_source_value')
...
class Metadata(SQLModel, table=True):
    ...
    metadata_concept_id: int = Field(sa_column=Column('metadata_concept_id', Integer, primary_key=True))
    metadata_type_concept_id: int = Field(sa_column=Column('metadata_type_concept_id', Integer, primary_key=True))
    name: str = Field(sa_column=Column('name', String(250), primary_key=True))
...
class ProcedureOccurrence(SQLModel, table=True):
    ...
    procedure_date: date = Field(sa_column=Column('procedure_date', Date), description='USER GUIDANCE: Use this date to determine the date the procedure occurred. | ETLCONVENTIONS: If a procedure lasts more than a day, then it should be recorded as a separate record for each day the procedure occurred, this logic is in lieu of the procedure_end_date, which will be added in a future version of the CDM.')
    ...
    procedure_datetime: Optional[datetime] = Field(default=None, sa_column=Column('procedure_datetime', DateTime), description=' | ETLCONVENTIONS: This is not required, though it is in v6. If a source does not specify datetime the convention is to set the time to midnight (00:00:0000)')
    ...
    procedure_type_concept: Optional['Concept'] = Relationship(back_populates='procedure_occurrence1')
...
class Specimen(SQLModel, table=True):
    ...
    specimen_type_concept_id: int = Field(sa_column=Column('specimen_type_concept_id', Integer), description=' | ETLCONVENTIONS: Put the source of the specimen record, as in an EHR system. [Accepted Concepts](https://athena.ohdsi.org/search-terms/terms?standardConcept=Standard&domain=Type+Concept&page=1&pageSize=15&query=).')
...
class VisitOccurrence(SQLModel, table=True):
    ...
    visit_end_datetime: Optional[datetime] = Field(default=None, sa_column=Column('visit_end_datetime', DateTime), description=' | ETLCONVENTIONS: If no time is given for the end date of a visit, set it to midnight (00:00:0000).')
    ...
    admitting_source_concept: Optional['Concept'] = Relationship(back_populates='visit_occurrence')
    care_site: Optional['CareSite'] = Relationship(back_populates='visit_occurrence')
    discharge_to_concept: Optional['Concept'] = Relationship(back_populates='visit_occurrence_')
...
class VisitDetail(SQLModel, table=True):
    ...
    visit_detail_end_datetime: Optional[datetime] = Field(default=None, sa_column=Column('visit_detail_end_datetime', DateTime), description=' | ETLCONVENTIONS: If no time is given for the end date of a visit, set it to midnight (00:00:0000).')
    ...
    admitting_source_concept_id: Optional[int] = Field(default=None, sa_column=Column('admitting_source_concept_id', Integer), description='USER GUIDANCE: Use this field to determine where the patient was admitted from. This concept is part of the visit domain and can indicate if a patient was admitted to the hospital from a long-term care facility, for example. | ETLCONVENTIONS: If available, map the admitted_from_source_value to a standard concept in the visit domain. [Accepted Concepts](https://athena.ohdsi.org/search-terms/terms?domain=Visit&standardConcept=Standard&page=1&pageSize=15&query=).')
    discharge_to_source_value: Optional[str] = Field(default=None, sa_column=Column('discharge_to_source_value', String(50)), description=' | ETLCONVENTIONS: This information may be called something different in the source data but the field is meant to contain a value indicating where a person was discharged to after a visit, as in they went home or were moved to long-term care. Typically this applies only to visits that have a length of stay of a day or more.')
    discharge_to_concept_id: Optional[int] = Field(default=None, sa_column=Column('discharge_to_concept_id', Integer), description='USER GUIDANCE: Use this field to determine where the patient was discharged to after a visit detail record. This concept is part of the visit domain and can indicate if a patient was discharged to home or sent to a long-term care facility, for example. | ETLCONVENTIONS: If available, map the DISCHARGE_TO_SOURCE_VALUE to a Standard Concept in the Visit domain. [Accepted Concepts](https://athena.ohdsi.org/search-terms/terms?domain=Visit&standardConcept=Standard&page=1&pageSize=15&query=).')
    ...
    admitting_source_concept: Optional['Concept'] = Relationship(back_populates='visit_detail')
    care_site: Optional['CareSite'] = Relationship(back_populates='visit_detail')
    discharge_to_concept: Optional['Concept'] = Relationship(back_populates='visit_detail_')
    ...
    visit_detail_parent: Optional['VisitDetail'] = Relationship(back_populates='visit_detail_parent_reverse')
    visit_detail_parent_reverse: List['VisitDetail'] = Relationship(back_populates='visit_detail_parent')
...
class DeviceExposure(SQLModel, table=True):
    ...
    unique_device_id: Optional[str] = Field(default=None, sa_column=Column('unique_device_id', String(50)), description='USER GUIDANCE: This is the Unique Device Identification number for devices regulated by the FDA, if given. | ETLCONVENTIONS: For medical devices that are regulated by the FDA, a Unique Device Identification (UDI) is provided if available in the data source and is recorded in the UNIQUE_DEVICE_ID field.')
...
class Measurement(SQLModel, table=True):
    ...
    measurement_concept: Optional['Concept'] = Relationship(back_populates='measurement')
    measurement_source_concept: Optional['Concept'] = Relationship(back_populates='measurement_')
    measurement_type_concept: Optional['Concept'] = Relationship(back_populates='measurement1')
    operator_concept: Optional['Concept'] = Relationship(back_populates='measurement2')
    ...
    unit_concept: Optional['Concept'] = Relationship(back_populates='measurement3')
    value_as_concept: Optional['Concept'] = Relationship(back_populates='measurement4')
...
class Note(SQLModel, table=True):
    ...
    note_type_concept: Optional['Concept'] = Relationship(back_populates='note2')
...
class Observation(SQLModel, table=True):
    ...
    observation_concept: Optional['Concept'] = Relationship(back_populates='observation')
    observation_source_concept: Optional['Concept'] = Relationship(back_populates='observation_')
    observation_type_concept: Optional['Concept'] = Relationship(back_populates='observation1')
    ...
    qualifier_concept: Optional['Concept'] = Relationship(back_populates='observation2')
    unit_concept: Optional['Concept'] = Relationship(back_populates='observation3')
    value_as_concept: Optional['Concept'] = Relationship(back_populates='observation4')
